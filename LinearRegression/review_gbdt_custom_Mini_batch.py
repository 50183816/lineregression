...
def fit(x,y,threshold=0e-5,max_iter=100,alpha=0.01):
    ...
    while jtheta_diff > threshold and iter < max_iter:
        ...
        for b in np.arange(m / 10,dtype=np.int):
            indexes = np.arange(b*10,np.min([b*10+10,m]),dtype=np.int32)
            for i in indexes:
                # print(i)
                tmp = 0
                hb = calcuateJ(x[i, :], theta)
...
t0 = time()
theta = fit(train_x,train_y,max_iter=1000,alpha=0.00032)
usedTime = time() - t0
...
plt.title('MiniBatch  GD score=%.3f,used time: %ss'%(r2,usedTime))
plt.show()