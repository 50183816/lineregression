...
def fit(X,Y,alpha=0.0001,fit_intercept=False,tol=1e-6,total_iter=2000):
    ...
    while jtheta_change > tol and iter < total_iter:
        ...
        for index in random_index:
            #计算预测值和实际值之间的差值
            y_predict = predict(theta,X[index],Y[index],intercept)
            y_true = Y[index]
            diff = y_true - y_predict
            for j in range(n):
                theta[j] += alpha*diff*X[index][j]
                if fit_intercept:
                    intercept = intercept + alpha * diff
...
if __name__ == '__main__':
    #pass
    theta,intercept,iter = fit(train_x,train_y,alpha=0.01,fit_intercept=True,total_iter=1000)
    ...
    print("sgd参数为{}".format(algo3.steps[-1][-1].coef_))
    print('截距项为{}'.format(algo3.steps[-1][-1].intercept_))
