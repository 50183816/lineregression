...
for file in file_labels:
       ...
       if flag < 200:
            emails.append(splittedcontent)
            email_labals.append(file[1])
       else:
           emails_test.append(splittedcontent)
...
emails = tfidf.fit_transform(emails)
print('size of emails before:{}'.format(np.shape(emails)))
svddecomposition = TruncatedSVD(n_components=20,random_state=22)
emails = svddecomposition.fit_transform(emails)
print('size of emails after:{}'.format(np.shape(emails)))
np.save('../datas/processedemails1',emails)
